...
} = require("../utils/windsurfProtobuf.js");

// P3: MITM debug capture — gated by env MITM_DEBUG=1, NODE_ENV=development, OR user config mitmDebug.
...
const { DATA_DIR: _DATA_DIR } = require("../paths");
function mitmDebugEnabled() {
  if (process.env.MITM_DEBUG === "1" || process.env.NODE_ENV === "development") return true;
  try { return loadConfig().mitmDebug === true; } catch { return false; }
}
let _dumpCounter = 0;
function dumpIntercept(stage, payload) {
  if (!mitmDebugEnabled()) return;
  try {
    const fs = require("fs");
    const path = require("path");
    const dumpDir = path.join(_DATA_DIR, "logs", "mitm");
    if (!fs.existsSync(dumpDir)) fs.mkdirSync(dumpDir, { recursive: true });
    _dumpCounter++;
    const ts = new Date().toISOString().replace(/[:.]/g, "-");
    const file = path.join(dumpDir, `windsurf-intercept-${ts}-${_dumpCounter}-${stage}.json`);
    // P1: redact sensitive fields (apiKey) before writing to disk.
    const REDACT_KEYS = new Set(["apiKey", "api_key", "authorization", "Authorization"]);
    const safePayload = (typeof payload === "string" || Buffer.isBuffer(payload))
      ? payload.toString("utf8")
      : JSON.stringify(payload, (k, v) => REDACT_KEYS.has(k) ? "[REDACTED]" : v, 2);
...
function translateToAnthropic(decoded, mappedModel) {
  ...
  // P5: guard negative/NaN values — fall back to defaults for invalid inputs.
  const safeNum = (v, fallback) => (typeof v === "number" && !isNaN(v) && v >= 0) ? v : fallback;
  if (cfg && safeNum(cfg.maxTokens, 0) > 0) {
    body.max_tokens = Math.min(safeNum(cfg.maxTokens, 128000), 128000);
  } else if (cfg && safeNum(cfg.maxNewlines, 0) > 0) {
  ...
  // P5: guard against negative/NaN — skip invalid params (let executor use defaults).
  if (cfg) {
    if (typeof cfg.temperature === "number" && !isNaN(cfg.temperature) && cfg.temperature >= 0) body.temperature = cfg.temperature;
    if (typeof cfg.topP === "number" && !isNaN(cfg.topP) && cfg.topP >= 0) body.top_p = cfg.topP;
    if (typeof cfg.topK === "number" && !isNaN(cfg.topK) && cfg.topK >= 0) body.top_k = cfg.topK;
  }
    ...
    switch (eventType) {
      ...
      case "message_delta": {
        ...
        if (delta.stop_reason || usage) {
          ...
          // P11: guard against negative token counts from malformed responses.
...
async function intercept(req, res, bodyBuffer, mappedModel, passthrough) {
  ...
  // 2. Resolve model: mappedModel → UI alias (from user config) → auto-map
  const { loadConfig } = require("../config");

...
export async function POST(request) {
  ...
  try {
    ...
    if (isOpenAICompatibleProvider(provider)) {
      ...
      const existingOAICompat = await getProviderConnections({ provider });
      if (existingOAICompat.length > 0) {
      ...
      const existingAnthropicCompat = await getProviderConnections({ provider });
      if (existingAnthropicCompat.length > 0) {
      ...
      const existingEmbeddingCompat = await getProviderConnections({ provider });
      if (existingEmbeddingCompat.length > 0) {

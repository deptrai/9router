 ...
 *
 * Ported from deepgrep/src/protobuf.mjs
 */
...
export function connectFrameDecode(data) {
  ...
  while (i + 5 <= data.length) {
    ...
    i += 5;
    // F18: Bounds check — skip truncated frame if length exceeds remaining data
    if (i + length > data.length) {
      // Truncated frame — emit remaining bytes as-is (best-effort, avoid garbage)
      const remaining = data.subarray(i);
      if (remaining.length > 0) frames.push(Buffer.from(remaining));
      break;
    }

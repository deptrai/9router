...
export class OrdersService {
  ...
  private getAdminService(): AdminOrdersService {
    return this.adminOrdersService ?? new AdminOrdersService(this.ledgerService, this.telegramBotService);
  }
  ...
  // ---------------------------------------------------------------------------
  // Story 5.3: Admin Orders Delegation (Refactored to AdminOrdersService per SRP)
  // ---------------------------------------------------------------------------

  /**
   * @deprecated Injected via AdminOrdersService directly. Retained for backward compatibility.
   */
  async listAdminOrders(
    query: ListAdminOrdersQueryDto,
    tx: DbOrTx = db,
  ): Promise<{ orders: AdminOrderListItemDto[]; total: number }> {
    return this.getAdminService().listAdminOrders(query, tx);
  }
  ...
  async getAdminOrderDetail(orderId: string, tx: DbOrTx = db): Promise<AdminOrderDetailDto> {
    return this.getAdminService().getAdminOrderDetail(orderId, tx);
  }
  ...
  async revealAdminCredential(
    ...
    return this.getAdminService().revealAdminCredential(orderId, adminId);
  }
  ...
  async adminManualRefund(
    ...
    return this.getAdminService().adminManualRefund(orderId, adminId, reason, markCredentialDefective, tx);
  }
  ...
  async notifyRefundCommit(
    ...
    return this.getAdminService().notifyRefundCommit(userId, productId, updatedOrder);
  }
}

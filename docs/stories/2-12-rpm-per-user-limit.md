...
- [ ] **A1**: `src/lib/quota/window.js` → thêm `"1m": 60 * 1000` vào `DURATIONS`. Không sửa logic `duration`/`resolveWindow`/`formatResetCountdown` (chúng generic theo type). Verify `formatResetCountdown` đọc ổn với khoảng < 1 phút (đã có nhánh `mins`, nhưng 1m thì human-readable nên là giây — xem A2).
- [ ] **A2**: (tùy chọn, nếu muốn message "reset after Xs") — `formatResetCountdown` hiện làm tròn theo phút (`reset after Xm`). Cho window 1m, đề xuất tự build chuỗi giây tại `rpmLimit.js` (`reset after ${sec}s`) thay vì dùng `formatResetCountdown`, để message chính xác giây. Ghi rõ quyết định trong comment. (KHÔNG đổi `formatResetCountdown` để tránh ảnh hưởng 5h/weekly.)

### Phần B — RPM repo/state (AC#2, #3, #7, #8)
- [ ] **B1**: Tạo scope kv cho RPM state: trong `quotaRepo.js` (hoặc module mới) thêm `makeKv("rpmState")` helper `getRpmState(userId)` / `setRpmState(userId, state)`. State shape đề xuất: `{ win1m: { startedAt: ISO, count: number } }`. (Giống `getQuotaState`/`setQuotaState` nhưng keyed theo `userId`, không phải `keyId`.)
- [ ] **B2**: Tạo `src/lib/quota/rpmLimit.js` → `checkRpmLimit(apiKey)`:
  - `if (!apiKey) return { allowed: true }`
  - `keyRow = await getApiKeyByKey(apiKey); if (!keyRow?.userId) return { allowed: true }` (legacy/không map)
  - `limits = await resolveUserLimits(keyRow.userId)`
  - `if (limits.source !== "plan" || !(limits.rpm > 0)) return { allowed: true }`
  - đọc state, `resolveWindow(state.win1m, "1m", now)` → nếu `reset` thì `count=0, startedAt=new`; else dùng count hiện tại
  - `if (count >= limits.rpm)` → tính `resetAt = startedAt + duration("1m")`, build human `reset after ${sec}s`, return `{ allowed:false, retryAfter: resetAt, retryAfterHuman, rpm: limits.rpm, count, planName: limits.planName }`
  - else: `count++`, persist state, return `{ allowed: true }`
  - **Bọc toàn bộ trong try/catch** → catch trả `{ allowed: true }` + `console.warn("[rpmLimit] fail-open:", e.message)` (AC#8).
- [ ] **B3**: Cân nhắc atomicity: read-modify-write counter có race khi nhiều request đồng thời (cùng user). MVP chấp nhận soft-limit (giống TOCTOU 10-key đã defer ở 2.3) — **ghi rõ trong comment** là known-limit, siết sau bằng transaction/atomic increment nếu cần. (KHÔNG block story vì điều này.)

### Phần C — Tích hợp handleChat (AC#4, #5, #6)
- [ ] **C1**: `src/sse/handlers/chat.js` → import `checkRpmLimit` từ `@/lib/quota/rpmLimit.js`. Trong `handleChat`, **sau** block `requireApiKey` + `if (!modelStr)` (dòng ~87) và **TRƯỚC** `handleBypassRequest`/combo branch (dòng ~89–95), chèn:
  ```js
  // --- RPM admission (Story 2.12, E.2) — count per-user, BEFORE combo expansion ---
  const rpm = await checkRpmLimit(apiKey);
  if (!rpm.allowed) {
    log.warn("RPM", `key="${log.maskKey(apiKey||"")}" RPM exceeded ${rpm.count}/${rpm.rpm} (plan ${rpm.planName}) ${rpm.retryAfterHuman||""}`);
    return unavailableResponse(HTTP_STATUS.RATE_LIMITED, `[plan ${rpm.planName}] RPM limit exceeded (${rpm.count}/${rpm.rpm})`, rpm.retryAfter, rpm.retryAfterHuman);
  }
  ```
  - **Vị trí chính xác**: trước `handleBypassRequest` (dòng 91) để warmup/naming bypass KHÔNG tốn RPM? → **Quyết định**: đặt RPM SAU `handleBypassRequest` (dòng ~92), để bypass request (naming/warmup) không tính RPM (chúng không phải request thật của user). Ghi rõ lý do trong comment.
  - Đảm bảo `unavailableResponse` + `HTTP_STATUS` đã import (đã có sẵn ở chat.js).
- [ ] **C2**: KHÔNG đụng `handleSingleModelChat` (RPM không enforce per-model). KHÔNG đụng `checkKeyQuota`/`checkCredits`. KHÔNG enforce quota 5h/weekly ở story này (đó là E.3).

### Phần D — Test (AC#9)
- [ ] **D1**: `tests/unit/window-1m.test.js` — `duration("1m")===60000`; `resolveWindow(null,"1m",t)` reset; `resolveWindow({startedAt: t-30s},"1m",t)` không reset; `resolveWindow({startedAt: t-61s},"1m",t)` reset. Verify 5h/weekly vẫn đúng (regression).
- [ ] **D2**: `tests/unit/rpm-limit.test.js` — dùng temp DB (pattern `resolve-plan.test.js`): tạo plan rpm=N, user có plan, tạo apiKey gắn userId. Test: (a) no apiKey → allow; (b) legacy key (userId null) → allow; (c) user source=credit (no plan) → allow; (d) plan rpm=0 → allow; (e) dưới limit → allow + count tăng; (f) chạm limit → `allowed:false` + retryAfter; (g) **multi-key gộp theo user**: 2 key cùng user, rpm=1 → req thứ 2 chặn; (h) sửa plan rpm rồi check lại → limit mới (AC#7); (i) fail-open: mock `resolveUserLimits` throw → allow; mock kv throw → allow.
- [ ] **D3**: `tests/unit/chatRpmCheck.test.js` — theo style `chatCreditCheck.test.js`: test logic admission (`!rpm.allowed` → gọi `unavailableResponse(429, msg, retryAfter, human)`; `allowed` → flow tiếp tục) + assert vị trí chèn là TRƯỚC combo (code-inspection note). KHÔNG cần full e2e handleChat.
- [ ] **D4**: Run full suite (`NODE_PATH=/tmp/node_modules npm test` từ `tests/`), 0 fail.

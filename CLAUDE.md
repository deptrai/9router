...
| 2026-06-29 | Dọn MCP trùng/lặp | Xóa codebase-memory-mcp khỏi opencode.json; xóa mcp-chrome khỏi .claude.json (trùng chrome-mcp trong devin config, cùng URL 127.0.0.1:12306) | Bỏ entry trùng lặp giữa các nguồn config |
| 2026-06-29 | Tune vibervn-context-engine | `agentic_rag_max_turns: 9` (default, giữ `agentic_rag: true`, `max_chunk_chars: 50000`, `grep_read: true`) | Test thực tế sau server restart sạch: max_turns=9 query phức tạp trả kết quả đầy đủ qua MCP, không hang. Các lần hang trước đây do server chạy binary không nhất quán sau nhiều lần build+restart thủ công |
| 2026-06-29 | Revert patch progress notifications | Revert toàn bộ patch progress channel (Peer extractor, notify_logging_message, sse_keep_alive tuning) trong vibervn-context-engine | Patch không hiệu quả vì `LoggingMessageNotification` route sang Common channel (client không đọc), `ProgressNotification` cần client gửi progressToken (Devin MCP client không gửi). rmcp default `sse_keep_alive=15s` đã đủ. Giữ code gốc |
